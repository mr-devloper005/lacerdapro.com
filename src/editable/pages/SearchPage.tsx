  ...
  return (
    <Link href={href} className={`group block overflow-hidden rounded-md border border-[var(--editable-border)] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-2xl ${strong ? 'md:col-span-2' : ''}`}>
      ...
      <div className="p-5 sm:p-6">
        ...
        <h2 className="mt-4 line-clamp-3 text-2xl font-extrabold leading-tight tracking-normal text-[var(--editable-page-text,#101827)]">{post.title}</h2>
...
export default async function SearchPage({ searchParams }: { searchParams?: Promise<{ q?: string; category?: string; task?: string; master?: string }> }) {
  ...
  return (
    <EditableSiteShell>
      <main className="min-h-screen bg-[var(--editable-page-bg,#eef7fc)] text-[var(--editable-page-text,#101827)]">
        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="grid gap-8 rounded-md border border-[var(--editable-border)] bg-white p-6 shadow-[0_22px_55px_rgba(15,23,42,0.10)] md:grid-cols-[0.8fr_1.2fr] lg:p-10">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.22em] text-[#0b965d]">{pagesContent.search.hero.badge}</p>
            ...
            <form action="/search" className="self-end rounded-md border border-[var(--editable-border)] bg-[#f6fbff] p-4 sm:p-5">
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                ...
                <select name="task" defaultValue={task} className="rounded-md border border-[var(--editable-border)] bg-white px-4 py-3 text-sm font-black outline-none">
                  <option value="">All content types</option>
                  {enabledTasks.map((item) => <option key={item.key} value={item.key}>{item.label}</option>)}
                </select>
              </div>
              <button className="mt-3 inline-flex h-12 w-full items-center justify-center rounded-md bg-[#0b965d] px-6 text-sm font-black uppercase tracking-[0.18em] text-white transition hover:-translate-y-0.5" type="submit">Search</button>
            </form>
          </div>
